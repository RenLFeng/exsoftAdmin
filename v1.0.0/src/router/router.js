import Vue from 'vue'
import Router from 'vue-router'
import store from '../store/store'
import NProgress from 'nprogress' //进度条
import 'nprogress/nprogress.css'
Vue.use(Router)
// 路由懒加载
const getComponent = (name, component) => () => import(`@/views/${name}/${component}.vue`);
const myRouter = new Router({
  // mode: 'history',
  // base:'/backend/',
  routes: [{
      path: '/',
      redirect: '/home',
      component: getComponent('login', 'index')
    },
    {
      path: '/ClassStatistics',
      name: 'ClassStatistics',
      component: getComponent('ClassStatistics', 'index'),
      children: [

      ]
    },
    {
      path: '/Rereport',
      name: 'Rereport',
      component: getComponent('ClassStatistics', 'Rereport'),
    },
    {
      path: '/Actreport',
      name: 'Actreport',
      component: getComponent('ClassStatistics', 'Actreport'),
    },
    {
      path: '/Analysis',
      name: 'Analysis',
      component: getComponent('ClassStatistics', 'Analysis'),
    },
    {
      path: '/login',
      name: 'login',
      component: getComponent('login', 'index')
    },
    {
      path: '/',
      component: getComponent('layout', 'Layout'),
      children: [{
          path: '/home',
          name: 'home',
          component: getComponent('home', 'index'),
          meta: {
            title: '首页'
          }
        },
        {
          path: '/Statistics',
          name: 'Statistics',
          component: getComponent('Statistics', 'index'),
          meta: {
            title: '学情统计'
          }
        },
        {
          path: '/allUser',
          component: getComponent('users', 'index'),
          name: 'allUser',
          meta: {
            title: '所有用户'
          }
        },
        {
          path: '/allBanke',
          component: getComponent('bankes', 'index'),
          name: 'allBanke',
          meta: {
            title: '班课管理'
          }
        },
        {
          path: '/allZuoye',
          component: getComponent('zuoye', 'index'),
          name: 'allZuoye',
          meta: {
            title: '作业管理'
          }
        },
        {
          path: '/allComment',
          component: getComponent('comment', 'allComment'),
          name: 'allComment',
          meta: {
            title: '所有评论'
          }
        },
        {
          path: '/allAnswer',
          component: getComponent('comment', 'allAnswer'),
          name: 'allAnswer',
          meta: {
            title: '所有回复'
          }
        },













        {
          path: '/icon',
          component: getComponent('excel', 'upload-excel'),
          name: 'UploadExcel',
          meta: {
            title: '自定义图标'
          }
        },
        {
          path: '/editor',
          component: getComponent('component', 'editor'),
          name: 'editor',
          meta: {
            title: '富文本编译器'
          }
        },
        {
          path: '/countTo',
          component: getComponent('component', 'countTo'),
          name: 'countTo',
          meta: {
            title: '数字滚动'
          }
        },
        {
          path: '/tree',
          component: getComponent('component', 'tree'),
          name: 'tree',
          meta: {
            title: '自定义树'
          }
        },
        {
          path: '/treeTable',
          component: getComponent('component', 'treeTable'),
          name: 'treeTable',
          meta: {
            title: '表格树'
          }
        },
        {
          path: '/treeSelect',
          component: getComponent('component', 'treeSelect'),
          name: 'treeSelect',
          meta: {
            title: '下拉树'
          }
        },
        {
          path: '/draglist',
          component: getComponent('draggable', 'draglist'),
          name: 'draglist',
          meta: {
            title: '拖拽列表'
          }
        },
        {
          path: '/dragtable',
          component: getComponent('draggable', 'dragtable'),
          name: 'dragtable',
          meta: {
            title: '拖拽表格'
          }
        },
        {
          path: '/cricle',
          component: getComponent('charts', 'cricle'),
          name: 'cricle',
          meta: {
            title: '饼图'
          }
        },
      ]
    }
  ]
})
myRouter.beforeEach((to, from, next) => {
  NProgress.start();
  // if (to.path !='/login' && !store.state.token) {
  //    next('/login')
  //    NProgress.done()
  // }else{
  //   next()
  // }
  if (to.path == '/ClassStatistics' || to.path == '/Rereport' || to.path == '/Actreport' || to.path == '/Analysis') {
    next();
  } else {
    if (to.path != '/login' && !store.state.token) {
      next('/login')
      NProgress.done()
    } else {
      next()
    }
  }
})
myRouter.afterEach(() => {
  NProgress.done()
})
export default myRouter